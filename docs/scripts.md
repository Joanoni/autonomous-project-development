...
4. `[4/5]` Run `cycle/main.py` to generate initial agent files.
...
**Purpose:** The core runtime script. Runs every time the Orchestrator starts its loop. Performs four steps in order:
...
4. **Scan inbox** — reads `agent_framework/inbox/unread/message.md` and returns the `to` field value.
...
sequenceDiagram
    ...
    loop Orchestrator Loop
        ...
        Note over CY: 4. Scan inbox/unread/message.md
