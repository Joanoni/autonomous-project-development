...
def main():
    ...
    try:
        metadata = parse_metadata(draft_message)
    ...
    print(f"[Info] Message validated - from: '{sender}', to: '{recipient}', subject: '{metadata['subject']}'")

    # 3. Archive current unread/ contents
    archive_unread(base_dir, sender, recipient)
